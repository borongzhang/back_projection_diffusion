...
class AnalyticalFstar(nn.Module):
  ...
  def __call__(self, inputs: jnp.ndarray) -> jnp.ndarray:
    """Applies the analytical back-scattering operator.

    Args:
      inputs: Input scattering data of shape [batch_size, 6400, 2],
        where the last dimension represents real and imaginary parts.

    Returns:
      jnp.ndarray: Output of shape [batch_size, 80, 80, 2] by applying the
        analytical back-scattering operator.
    """
...
class EquinetFstar(nn.Module):
  ...
  def setup(self) -> None:
    ...
    self.cos_kernel1 = self.param('cos_kernel1', nn.initializers.uniform(),
                                  kernel_shape)
    self.sin_kernel1 = self.param('sin_kernel1', nn.initializers.uniform(),
                                  kernel_shape)
    self.cos_kernel2 = self.param('cos_kernel2', nn.initializers.uniform(),
                                  kernel_shape)
    self.sin_kernel2 = self.param('sin_kernel2', nn.initializers.uniform(),
                                  kernel_shape)
    self.cos_kernel3 = self.param('cos_kernel3', nn.initializers.uniform(),
                                  kernel_shape)
    self.sin_kernel3 = self.param('sin_kernel3', nn.initializers.uniform(),
                                  kernel_shape)
    self.cos_kernel4 = self.param('cos_kernel4', nn.initializers.uniform(),
                                  kernel_shape)
    self.sin_kernel4 = self.param('sin_kernel4', nn.initializers.uniform(),
                                  kernel_shape)

  def __call__(self, inputs: jnp.ndarray) -> jnp.ndarray:
    """Applies the equivariantly approximated back-scattering operator.
    ...
    Returns:
      jnp.ndarray: Output of shape [batch_size, 80, 80, 1] after the operator.
    """
    ...
    output_polar = (helper(self.pre1, self.post1, self.cos_kernel1,
                             self.cos_kernel2, Rs)
                    + helper(self.pre2, self.post2, self.sin_kernel1,
                             self.sin_kernel2, Rs)
                    + helper(self.pre3, self.post3, self.cos_kernel3,
                             self.sin_kernel3, Is)
                    + helper(self.pre4, self.post4, self.sin_kernel4,
                             self.cos_kernel4, Is))
    output_polar = jnp.reshape(output_polar, (-1, self.nx**2, 1))

    def polar_to_cart(x: jnp.ndarray) -> jnp.ndarray:
      x = self.cart_mat @ x
...
def build_permutation_indices(L: int, l: int) -> jnp.ndarray:
  """Builds permutation indices for redistributing blocks.

  Args:
    L: Global level parameter.
    l: Local level parameter.

  Returns:
    jnp.ndarray: Array of permutation indices.
  """
  delta = 2 ** (L - l - 1)
...
def build_switch_indices(L: int) -> jnp.ndarray:
  """Precomputes indices for redistributing blocks according to x -> M*xM."""
  L = L // 2
...
class V(nn.Module):
  ...
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    """Processes input using learned complex-valued weights."""
...
class H(nn.Module):
  ...
  @nn.compact
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    """Applies complex-valued weights after permutation and reshaping."""
    m = x.shape[1] // 2
    s = x.shape[2] * 2
    init_fn = nn.initializers.glorot_uniform()
...
class M(nn.Module):
  @nn.compact
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    """Applies a residual mapping with complex weights."""
...
class G(nn.Module):
  ...
  @nn.compact
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    """Processes input with complex weights and applies permutation."""
    m = x.shape[1] // 2
    s = x.shape[2] * 2
    init_fn = nn.initializers.glorot_uniform()
...
class U(nn.Module):
  ...
  @nn.compact
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    """Applies learned weights and returns the final output in cartesian space."""
    n, r = x.shape[1], x.shape[2]
    nx = n * self.s
    init_fn = nn.initializers.glorot_uniform()
...
class BEquinetFstar(nn.Module):
  ...
  def setup(self) -> None:
    ...
    self.Gs = [
      G(build_permutation_indices(self.L, l))
  ...
  def __call__(self, inputs: jnp.ndarray) -> jnp.ndarray:
    """Applies the equivariantly approximated back-scattering operator and
    compresses via butterfly factorization.
    Args:
      inputs: Input scattering data of shape [batch_size, 6400, 2],
        where the last dimension represents real and imaginary parts.
    Returns:
      jnp.ndarray: Output of shape [batch_size, 80, 80, 1] in cartesian space.
    """
...
class DMLayer(nn.Module):
  ...
  @nn.compact
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    """Applies a dense mapping layer with a bias.
    Args:
      x: Input of shape [batch_size, a, b].
    Returns:
      jnp.ndarray: Output of shape [batch_size, a, output_dim].
    """
    kernel_shape = (x.shape[-2], x.shape[-1], self.output_dim)
    kernel = self.param('kernel', nn.initializers.uniform(), kernel_shape)
    bias_shape = (1, x.shape[-2], self.output_dim)
...
class SwitchNetFstar(nn.Module):
  L1: int
  L2x: int
  L2y: int
  Nw1: int
  ...
  @nn.compact
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    """Applies the back-scattering operator approximated by SwitchNet.
    Args:
      x: Input scattering data of shape [batch_size, 6400, 2],
        where the last dimension represents real and imaginary parts.
    Returns:
      jnp.ndarray: Output of shape [batch_size, 80, 80, 2].
    """
    ...
    x = x.transpose((0, 1, 3, 2, 4, 5))
    x = x.reshape((batch_size, self.Nb2x * self.Nw2x,
                   self.Nb2y * self.Nw2y, 2))
    return x
